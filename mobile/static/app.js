...
function configuredApiBase() {
  const raw = String(window.ANAV1_CONFIG?.apiBaseUrl || "").trim();
  return raw.replace(/\/+$/, "");
}
...
function renderRecording(recording) {
  ...
  const recordingAudioUrl = apiUrl(recording.audio_url);
  elements.audioPlayer.src = recordingAudioUrl;
  ...
  drawWaveformFromUrl(recordingAudioUrl, recording.original_filename || "Loaded audio").catch(() => {
